...
namespace iGeospatial.Coordinates.Transforms
{
	...
	internal class QRDecomposition
	{
		...
		/// <summary>
		/// Row and column dimensions. m is column dimension, n is row dimension.
		/// </summary>
		private int m, n;
		...
		/// <summary>
		/// Initializes a new QR Decomposition, computed by Householder reflections.
		/// </summary>
        /// <param name="A">
		...
		public QRDecomposition(GeneralMatrix A)
		{
			...
			for (int k = 0; k < n; k++)
			{
				...
				for (int i = k; i < m; i++)
				{
					nrm = GeneralMatrix.Hypot(nrm, QR[i][k]);
				}
		...
		/// <summary>Determines whether the matrix is full rank.</summary>
		/// <value>true if R, and hence A, has full rank.</value>
		...
		/// <value> 
		/// A <see cref="GeneralMatrix"/> of the upper triangular factor.
		/// </value>
		...
		/// </summary>
		/// <value> A <see cref="GeneralMatrix"/> of orthogonal factor.</value>
		public GeneralMatrix Q
		{
			get
			{
				GeneralMatrix X = new GeneralMatrix(m, n);
